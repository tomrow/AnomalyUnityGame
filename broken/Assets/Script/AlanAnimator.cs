...
public class AlanAnimator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        stepSoundSrc = stepSoundObj.GetComponent<AudioSource>();
    ...
    void FixedUpdate()
    {
        ...
        animationID = thePlayer.GetComponent<PlayerMovement>().playerActionMode;
        vSpeed = thePlayer.GetComponent<PlayerMovement>().vspeed;
        animationID += 0;
        //Debug.Log(Time.fixedDeltaTime);
        //pNeck.localPosition = new Vector3(0, 0, 0);
        //if (WaitInc > 600) { incrementBy += 0.000002f; }
        //else { WaitInc++; }
        incrementBy = Convert.ToSingle(Math.Floor(thePlayer.GetComponent<PlayerMovement>().stickPushedFromCenter * animationMultiplier) * Time.fixedDeltaTime)*100; //* animSpeedModifierFpsAffected; //For run animation
        //*(incrementBy*1.4)
        ...
        if (animationID == 0)  //run animation
        {
            ...
            if (incrementBy != 0f)
            {
                ...
                if(pTorso.localPosition.z < 0.05)
                {
                    if (!footstep)
                    {
                        stepSoundSrc.Stop();
                        stepSoundSrc.pitch = (Convert.ToSingle(Random.Range(150, 100))/100f);
                        stepSoundSrc.Play(); 
                ...
                {
                    pRightShoulder.localRotation = Quaternion.Euler(0, -45, Convert.ToSingle(Math.Sin(runAnimRadians) * 40));
                    pLeftShoulder.localRotation = Quaternion.Euler(0, 45, Convert.ToSingle(Math.Sin(runAnimRadians) * 40));
                    ...
                    pNeck.localRotation = Quaternion.Euler(0, 0, Mathf.Sin(runAnimRadians) * -30); //originally 30
                    ...
                    pTorso.localPosition = new Vector3(0, 0, Convert.ToSingle(Math.Abs(Math.Sin(runAnimRadians)) / 4));
